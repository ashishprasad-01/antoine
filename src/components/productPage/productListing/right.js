...
export default function Right() {
  ...
  const addToWishList = async (title, author, img, price) => {
    let { data } = await axios.post(
      ...
      <div className={book.rightSide}>
        <div className={book.topLeft}>
          <p>
            {t("Books")}
            <span>
              {records.length} {t("of")} {allproducts.length}
            </span>
          </p>
          ...
          <div className={book.topRight}>
            <form onChange={handleSubmit(submit)}>
              <select {...register("sortProducts")} defaultValue={"name_a_z"}>
                <option value="name_a_z">{t("Sort By A to Z")}</option>
        ...
        {allproducts.length > 0 ? (
          ...
          <h2 style={{ marginTop: "20px" }}>No Products are available</h2>
        )}
